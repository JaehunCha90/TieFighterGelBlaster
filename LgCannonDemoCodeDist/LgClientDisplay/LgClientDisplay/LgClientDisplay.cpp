...
#define IDC_LABEL_USER_ID         1027
#define IDC_LABEL_PASSWORD        1028
#define IDC_EDIT_USER_ID          1029
#define IDC_EDIT_PASSWORD         1030
#define IDC_CHECKBOX_CAMERA_ENABLE 1031
#define IDM_ADMIN                 1032
...
static char RemoteAddress[512]="192.168.0.248";
static char EngagementOrder[512] = "0123456789";
static char PreArmCode[512] = "";
static int authFailCount = 0;
static PROCESS_INFORMATION pi;
static char StatusSysem[512] = "";
static char Txt_UserID[512] = "";
static char Txt_Password[512] = "";
bool CameraOn = true;
...
static void SetStdOutToNewConsole(void)
{
    ...
    retVal = freopen_s(&pCout, "CONOUT$", "w", stdout);
    retVal = freopen_s(&pCout, "CONIN$", "r", stdin);
}
...
{
    static unsigned char KeyInput = 0;
    switch (message)
    {
    case WM_LBUTTONDOWN:
       {
    ...
    case WM_KEYDOWN:
        {
        HWND hEditWnd;
        hEditWnd = GetDlgItem(hWnd, IDC_SYSTEM_STATE);
        ...
        if (calib)
        {
            ...
            if (SendCode != 0)
            {
                ...
                else
                {
                    if (KeyInput == 0)
                    {
                        GetWindowTextA(hEditWnd, StatusSysem, sizeof(StatusSysem));
                    }
                    KeyInput = 1;
                    ...
                    {
                    case DEC_X:
                        SetWindowTextA(hEditWnd, "Moving Left..");
                        break;
                    case INC_X:
                        SetWindowTextA(hEditWnd, "Moving Right..");
                        break;
                    case INC_Y:
                        SetWindowTextA(hEditWnd, "Moving Down..");
                        break;
                    case DEC_Y:
                        SetWindowTextA(hEditWnd, "Moving Up..");
                        break;
                    }
                }
            }
        ...
        else
        {
            ...
            if (SendCode != 0)
            {
                ...
                else if (SendCode != FIRE_START)
                {
                    ...
                    SetWindowTextA(hEditWnd, "Moving ");
                    switch (SendCode)
                    {
                    case DEC_X:
                        SetWindowTextA(hEditWnd, "Moving Left..");
    ...
    case WM_KEYUP:
       {
        ...
        if (SendCode != 0)
        {
            ...
            if (SystemState & SAFE);
            else if (SendCode != FIRE_STOP)
            {
                SetWindowTextA(GetDlgItem(hWnd, IDC_SYSTEM_STATE), StatusSysem);
            }
        }
       }
       break;
    case WM_COMMAND:
        {
            ...
            case IDM_CONNECT:
                if (OnConnect(hWnd, message, wParam, lParam))
                {
                    ...
                    SendMessage(hWndMainToolbar, TB_SETSTATE, IDM_ADMIN,
                        (LPARAM)MAKELONG(TBSTATE_ENABLED, 0));

                    EnableWindow(GetDlgItem(hWnd, IDC_EDIT_USER_ID), false);
            ...
            case IDM_DISCONNECT:
                ...
                SetWindowTextA(hTempWnd, "Disconnected..");
                SendMessage(hWndMainToolbar, TB_SETSTATE, IDM_CONNECT,
                    (LPARAM)MAKELONG(TBSTATE_ENABLED, 0));
                SendMessage(hWndMainToolbar, TB_SETSTATE, IDM_DISCONNECT,
                    (LPARAM)MAKELONG(TBSTATE_INDETERMINATE, 0));
            ...
            case IDM_ADMIN:
                {
                    const TCHAR* url = _T("https://10.8.0.1:9090");

                    TCHAR chromePath[] = _T("C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe");
                    TCHAR args[1024];
                    _stprintf_s(args, _T("--ignore-certificate-errors --lang=en-US --user-data-dir=C:\\Temp\\ChromeProfile --app=%s"), url);

                    ShellExecute(NULL, _T("open"), chromePath, args, NULL, SW_SHOWNORMAL);
                }
                break;

            case IDC_EDIT_USER_ID:
                {
                    HWND hEditWnd;
                    hEditWnd = GetDlgItem(hWnd, IDC_EDIT_USER_ID);
                    GetWindowTextA(hEditWnd, Txt_UserID, sizeof(Txt_UserID));
                }
                break;
            ...
            case IDC_EDIT_PASSWORD:
                {
                    HWND hEditWnd;
                    hEditWnd = GetDlgItem(hWnd, IDC_EDIT_PASSWORD);
                    GetWindowTextA(hEditWnd, Txt_Password, sizeof(Txt_Password));
                }
                break;
            default:
    ...
    case WM_SYSTEM_STATE:
        {
          ...
          case SAFE:
              ...
              EnableWindow(GetDlgItem(hWnd, IDC_CHECKBOX_CAMERA_ENABLE), false);
          ...
          case PREARMED:
              ...
              EnableWindow(GetDlgItem(hWnd, IDC_CHECKBOX_CAMERA_ENABLE), true);
              CheckDlgButton(hWnd, IDC_CHECKBOX_ARMED_MANUAL, BST_UNCHECKED);
              CheckDlgButton(hWnd, IDC_CHECKBOX_LASER_ENABLE, BST_UNCHECKED);
              CheckDlgButton(hWnd, IDC_CHECKBOX_CALIBRATE, BST_UNCHECKED);
              CheckDlgButton(hWnd, IDC_CHECKBOX_AUTO_ENGAGE, BST_UNCHECKED);
          ...
          case ENGAGE_AUTO:
              if (SystemState & ARMED)
              {
                  ...
                  else
                  {
                      ...
                      SetWindowTextA(hTempWnd, "Searching for Target");
                  }
              }
              else
              {
...
HWND CreateSimpleToolbar(HWND hWndParent)
{
    ...
    TBBUTTON tbButtons[numButtons] =
    {
        ...
        { MAKELONG(13,         ImageListID), IDM_ADMIN,       TBSTATE_INDETERMINATE,       buttonStyles, {0}, 0, (INT_PTR)L"Admin" }
    };
...
static LRESULT OnCreate(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    CreateWindow(_T("STATIC"),
		...
		CreateWindow(_T("button"), _T("Camera Enable"),
            WS_VISIBLE | WS_CHILD | BS_CHECKBOX,
            670, 75, 125, 20,
            hWnd, (HMENU)IDC_CHECKBOX_CAMERA_ENABLE, ((LPCREATESTRUCT)lParam)->hInstance, NULL);
        CheckDlgButton(hWnd, IDC_CHECKBOX_CAMERA_ENABLE, checked);
        EnableWindow(GetDlgItem(hWnd, IDC_CHECKBOX_CAMERA_ENABLE), false);
    ...
    TmpHandle = CreateWindowExA(WS_EX_CLIENTEDGE,
            ...
            980, 50, 240, 20,
                    hWnd,
                    (HMENU)IDC_EDIT_USER_ID,
                    ((LPCREATESTRUCT)lParam)->hInstance, NULL);
        style = GetWindowLong(TmpHandle, GWL_STYLE);
        SetWindowLong(TmpHandle, GWL_STYLE, style);
        EnableWindow(GetDlgItem(hWnd, IDC_EDIT_USER_ID), true);

    // New Password Label and Edit Field
    CreateWindow(_T("STATIC"),
                 _T("Password:"),
                 WS_VISIBLE | WS_CHILD,
            900, 75, 75, 20,
...
#if 0
BOOL connectVPN(std::string userid, std::string passwd)
{
    // start openvpn
    HANDLE cout_r = NULL;
    HANDLE cout_w = NULL;
    ...
    CreatePipe(&cin_r, &cin_w, &sec_a, 0);
    SetHandleInformation(cout_r, HANDLE_FLAG_INHERIT, 0);
    SetHandleInformation(cin_w, HANDLE_FLAG_INHERIT, 0);
    ...
    // Todo..
    // how can I find proper location of openvpn.exe?
    wchar_t exeName[] = L".\\openvpn\\openvpn.exe --config .\\openvpn\\config\\client.ovpn";

    if (!CreateProcess(NULL, exeName, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi))
    {
        printf("cannot run openvpn..(%d)\n", GetLastError());
        return FALSE;
    }
    else {
    ...
    std::string line;
    std::string connection_success = "Initialization Sequence Completed";
    std::string auth_failed = "Received control message: AUTH_FAILED";

    int file_descripter = _open_osfhandle((intptr_t)cout_r, 0);
    FILE* file = _fdopen(file_descripter, "r");
    ...
    passwd.push_back('\n');
    Sleep(500);
    WriteFile(cin_w, userid.c_str(), static_cast<DWORD>(userid.size()), NULL, NULL);
    Sleep(500);
    WriteFile(cin_w, passwd.c_str(), static_cast<DWORD>(passwd.size()), NULL, NULL);
    Sleep(500);

    // Todo. time out or retry count..
    for (;;) {
        std::getline(file_stream, line);
        std::cout << line << std::endl;

        if (line.find(connection_success) != std::string::npos) {
            Sleep(1000);
            return TRUE;
        }
        else if (line.find(auth_failed) != std::string::npos)
        {
            // Todo.. checking is needed. openvpn process is finished or not...
            authFailCount += 1;
            // wait for closing openvpn..
...
BOOL connectVPN(HWND hWnd, std::string address, std::string userid, std::string passwd)
{
    // VPN 연결 중 대기 메시지 박스 표시를 위한 변수
    bool vpnConnectionCompleted = false;
    bool connectionResult = false;
    std::wstring ipAddr(address.begin(), address.end());

    // VPN 연결 로직을 별도 스레드에서 실행
    auto vpnThread = std::thread([&ipAddr, &userid, &passwd, &vpnConnectionCompleted, &connectionResult]() {
        // start openvpn
        HANDLE cout_r = NULL;
        HANDLE cout_w = NULL;
        HANDLE cin_r = NULL;
        ...
        wchar_t exeName[512] = L".\\openvpn\\openvpn.exe --config .\\openvpn\\config\\client.ovpn";
        wchar_t remoteAddr[256] = L" --remote ";
        wcsncat_s(remoteAddr, 256, ipAddr.c_str(), _TRUNCATE);
        wcsncat_s(exeName, 512, remoteAddr, _TRUNCATE);

        if (!CreateProcess(NULL, exeName, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL, NULL, &si, &pi)) {
            printf("cannot run openvpn..(%d)\n", GetLastError());
            vpnConnectionCompleted = true;
            return;
        }
        else {
            CloseHandle(cin_r);
        ...
        for (;;) {
            ...
            if (line.find(connection_success) != std::string::npos) {
                ...
                MessageBox(NULL, L"VPN connection successful", L"VPN Success", MB_OK | MB_ICONINFORMATION);
                return;
            }
            else if (line.find(auth_failed) != std::string::npos) {
                authFailCount += 1;
                ...
                ZeroMemory(&pi, sizeof(pi));
                vpnConnectionCompleted = true;
                MessageBox(NULL, L"VPN connection failed", L"VPN Failed", MB_OK | MB_ICONINFORMATION);
                return;
            }
            ++retryCount;
        }
    ...
    // 메인 스레드에서 메시지 박스 표시
    while (!vpnConnectionCompleted) {
        MessageBox(hWnd, L"Connecting to VPN... Please wait for a moment.", L"VPN Connecting", MB_OK | MB_ICONINFORMATION);
    }
...
static int OnConnect(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (!IsClientConnected())
    {
        ...
        if (!VerifyFileIntegrity()) {
            PRINT(_T("File Contamination Detected!!!\r\n"));
            DisplayMessageOkBox("File Contamination Detected!!!");
            return 0;
        }

        std::string uid = Txt_UserID;
        std::string passwd = Txt_Password;
        if (!connectVPN(hWnd, RemoteAddress, uid, passwd)) {
            std::cout << "Secure Network Connection Failed" << std::endl;
            if (authFailCount >= 3) {
                DisplayMessageOkBox("Authrization Failed more than 3 times..\nplease wait 1 hours");
            }
            else {
                DisplayMessageOkBox("Secure Network Connection Failed!\nAuthorization Failed..");
            }
        ...
        if (ConnectToSever("10.8.0.1", VIDEO_PORT))
        {
            ...
            if (ConnectToSeverForAlive("10.8.0.1", ALIVECOMM_PORT))
            {
                std::cout << "Start to send heartbeat for alive" << std::endl;
                StarAliveComm();
            }
