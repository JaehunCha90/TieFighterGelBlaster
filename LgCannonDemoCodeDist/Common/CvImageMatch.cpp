...
int LoadRefImages(Symbol* symbols) {
    ...
    for (int i = 0; i < NUM_SIGNS; i++)
    {
        ...
        char name[255] = {0};
        snprintf(filename, sizeof(filename) - 1, "%sT%d.jpg", IMAGE_DIR, i);
        snprintf(name, sizeof(name) - 1, "%d", i);
...
void WriteFile(const  char* path, const char* folder, const char* filename, char * ext, const cv::Mat src)
{
    ...
    FILE* f;
    char PathFilename_xml[PATH_MAX] = {0};
    char PathFilename_jpg[PATH_MAX] = {0};
    char FullPath[PATH_MAX] = {0};

    snprintf(PathFilename_xml, sizeof(PathFilename_xml) - 1, "%s%s/%s.xml", path, folder, filename);
    snprintf(PathFilename_jpg, sizeof(PathFilename_jpg) - 1, "%s%s/%s.%s", path, folder, filename, ext);
#if DEBUG
    printf("%s\n", PathFilename_jpg);
#endif
#if  defined(_WIN32) || defined(_WIN64)
    ...
    qsort(DetectedMatches, NumMatches, sizeof(struct NDetectedMatches), compare);
        ...
        for (int i = 0; i < NumMatches; ++i)
        {
            ...
            int ymin = (int)DetectedMatches[i].ymin;
            int ymax = (int)DetectedMatches[i].ymax;
            snprintf(name, sizeof(name) - 1, "%d", match);
